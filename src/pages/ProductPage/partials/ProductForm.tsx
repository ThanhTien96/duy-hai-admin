...
const ProductForm = ({
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...
      maDanhMucNho:
        defaultValue && defaultValue.maDanhMucNho
          ? defaultValue.maDanhMucNho
          : "",
      seoTitle:
        defaultValue && defaultValue.seoTitle ? defaultValue.seoTitle : "",
      seoDetail:
        defaultValue && defaultValue.seoDetail ? defaultValue.seoDetail : "",
      youtubeVideo:
        defaultValue && defaultValue.youtubeVideo
        ...
        <Row gutter={[32, 32]}>
          ...
          <Col span={24} xxl={12}>
            ...
            <Form.Item required label="Giá Gốc">
              <InputNumber
                value={
                  values.giaGoc && values.giaGoc > 0 ? values.giaGoc : undefined
                }
            ...
            <Form.Item required label="Tổng Số Lượng">
              <InputNumber
                value={
                  values.tongSoLuong && values.tongSoLuong > 0
                    ? values.tongSoLuong
                    : undefined
                }
                addonAfter={CURRENCY.vnd}
            ...
            <Form.Item required label="Danh Mục">
              <Select
                value={
                  values.maDanhMucNho.length > 0
                    ? values.maDanhMucNho
                    : undefined
                }
                onChange={(value: string) =>
                  setFieldValue("maDanhMucNho", value)
                }
                options={
                  subCategories &&
            ...
            <Form.Item label="Hình Ảnh" required>
              ...
              {!values.hinhAnh && defaultValue && (
                <Space className="mb-4">
                  {defaultValue.hinhAnh.map((img: IProductMediaType) => (
                    <Image
                      key={img.id}
                      width={100}
                      height={100}
                      src={img.hinhAnh}
                      alt={defaultValue.tenSanPham}
                    />
                  ))}
                </Space>
              )}
